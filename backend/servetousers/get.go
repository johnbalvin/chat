...
func Get() (*template.Template, map[string][]byte, map[string]SWInfo) {
	...
	if err != nil {
		log.Fatal("servetousers -> Get:1 -> err:", err)
	}
	if err = doc.DataTo(&htmlAux); err != nil {
		log.Fatal("servetousers -> Get:2 -> err:", err)
	}
	...
	for path := range htmlAux {
		go func(path string) {
			...
			if err := htmlInfo.getHTMLSW(); err != nil {
				log.Fatal("servetousers -> Get:3 -> ruta:", htmlInfo.Path, " err:", err)
			}
			htmlInfo.Path = htmlInfo.Path[6:]
	...
	if err != nil {
		log.Fatal("servetousers -> Get:4 -> err:", err)
	}
	if err = doc.DataTo(&serviceWorkersAux); err != nil {
		log.Fatal("servetousers -> Get:5 -> err:", err)
	}
	...
	for checksum := range serviceWorkersAux {
		go func(checksum string) {
			...
			if err := getSW(id, &swinfo); err != nil {
				log.Fatal("servetousers -> Get:6 -> err:", err)
			}
...
func getSW(id string, static *SWInfo) error {
	...
	if err != nil {
		log.Fatal("servetousers ->  getSW:1 -> err:", err)
		return nil
	}
	static.Me, err = ioutil.ReadAll(reader)
	if err != nil {
		log.Fatal("servetousers -> getSW:2 -> err:", err)
		return nil
	}
	if err = reader.Close(); err != nil {
		log.Fatal("servetousers -> getSW:3 -> err:", err)
		return nil
	}
	return nil
}
func (htmlInfo *HTMLInfo) getHTMLSW() error {
	...
	if err != nil {
		log.Fatal("servetousers -> (htmlInfo *HTMLInfo) getHTMLSW:1 -> err:", err)
		return err
	}
	htmlInfo.Content, err = ioutil.ReadAll(reader)
	if err != nil {
		log.Fatal("servetousers -> (htmlInfo *HTMLInfo) getHTMLSW:2 -> err:", err)
		return err
	}
	if err = reader.Close(); err != nil {
		log.Fatal("servetousers -> (htmlInfo *HTMLInfo) getHTMLSW:3 -> err:", err)
		return err
	}
	return nil
}
